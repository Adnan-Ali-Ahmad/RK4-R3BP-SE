...
double planet_accretion(object &m,object &COM){
//return 0.;
    double vx = m.vx - COM.vx;
    double vy = m.vy - COM.vy;
    double vz = m.vz - COM.vz;
    double v2 = vx*vx+vy*vy+vz*vz;
    double RA = 2.*G*m.m/(v2);
    double res = pi*RA*RA*m.rho*sqrt(v2);
    return res;
}

double planet_evap(object &m){
//return 0.;
    double E = G*m.m*mh/m.R/(k*m.T);
    double v0 = sqrt(2.*k*m.T/mh);
    double S = 4.*pi*m.R*m.R;
    double res = -S*nh*mh*v0*(1.+E)*exp(-E)/2./sqrt(pi);
    return res;
}

double Fgx(object &m,object &COM){
//return 0.;
    double vx = m.vx-COM.vx;
    double vy = m.vy-COM.vy;
    double vz = m.vz-COM.vz;
    double v = sqrt(vx*vx+vy*vy+vz*vz);
    return -4.*pi*(G*G*m.m/(cs*cs))*m.rho*I*vx/v;
}

double Fgy(object &m,object &COM){
//return 0.;
    double vx = m.vx-COM.vx;
    double vy = m.vy-COM.vy;
    double vz = m.vz-COM.vz;
    double v = sqrt(vx*vx+vy*vy+vz*vz);
    double res = -4.*pi*(G*G*m.m/(cs*cs))*m.rho*I*vy/v;
    return res;
}

double Fgz(object &m,object &COM){
//return 0.;
    double vx = m.vx-COM.vx;
    double vy = m.vy-COM.vy;
    double vz = m.vz-COM.vz;
    double v = sqrt(vx*vx+vy*vy+vz*vz);
    return -4.*pi*(G*G*m.m/(cs*cs))*m.rho*I*vz/v;
}

double Tp(object &m,object &CB){
//return 0.;
    double L = sigma*CB.T*CB.T*CB.T*CB.T*4.*pi*CB.R*CB.R;
    double x = m.x - CB.x;
    double y = m.y - CB.y;
    double z = m.z - CB.z;
    double r2 = x*x+y*y+z*z;
    double T4 = L*(1.-A)/(16.*pi*r2*sigma);
    return pow(T4,1./4.);
}

double Tsh(object &m, object &CB){
//return 0.;
    double vx = m.vx-CB.vx;
    double vy = m.vy-CB.vy;
    double vz = m.vz-CB.vz;
    double v2 = vx*vx+vy*vy+vz*vz;
    double res = 3.*mh*v2/(16.*k);
    return res;
}

double get_rho(object &m,parameters &P){
//return 0.;
    if(P.mls == 0.){
        return 1e-7;
    }
    double res = P.mls/(4.*pi*m.a*m.a*P.vwind)/31557600.0;
    return res;
}

double Ff(const object &m,const object &CB,const parameters &P,int component){
//return 0.;
    double vx = m.vx-CB.vx;
    double vy = m.vy-CB.vy;
    double vz = m.vz-CB.vz;
    ...
    double f;
    if(component == 0){
        f = -0.5*m.rho*P.C*v*vx*pi*m.R*m.R;
    }
    if(component == 1){
        f = -0.5*m.rho*P.C*v*vy*pi*m.R*m.R;
    }
    if(component == 2){
        f = -0.5*m.rho*P.C*v*vz*pi*m.R*m.R;
    }
    double res = f/m.m;
    return res;
}

void RK4(object &m1, object &m2,object &M1,object &M2,object M3[], object ast[],distances SIM[],parameters P,int N_ast, double dt){
    ...
    for(int i=1;i<N_ast;i++){
        ...
        M3vz[i] = ast[i].vz;
        M3m[i] = ast[i].m;
        M3[i].a = rij(M3[i],M1); M3[i].rho = get_rho(M3[i],P);
        SIM[i].r12 = SIM[0].r12;
    ...
    double k1m3[N_ast], k2m3[N_ast], k3m3[N_ast], k4m3[N_ast];
    ...
    k1x2 = dt*(ax2+Ff(M2,M1,P,0)+Fgx(M2,M1));
    k1y2 = dt*(ay2+Ff(M2,M1,P,1)+Fgy(M2,M1));
    k1z2 = dt*(az2+Ff(M2,M1,P,2)+Fgz(M2,M1));
    l1x2 = dt*M2.vx;
    l1y2 = dt*M2.vy;
    l1z2 = dt*M2.vz;
    k1m2 = dt*(planet_accretion(M2,M1)+planet_evap(M2));

    for(int i=0;i<N_ast;i++){
        ax3 = acc3x(M1,M2,M3[i],SIM[i]);
        ay3 = acc3y(M1,M2,M3[i],SIM[i]);
        az3 = acc3z(M1,M2,M3[i],SIM[i]);
        k1x3[i] = dt*(ax3+Ff(M3[i],M1,P,0)+Fgx(M3[i],M1));
        k1y3[i] = dt*(ay3+Ff(M3[i],M1,P,1)+Fgy(M3[i],M1));
        k1z3[i] = dt*(az3+Ff(M3[i],M1,P,2)+Fgz(M3[i],M1));
        l1x3[i] = dt*M3[i].vx;
        l1y3[i] = dt*M3[i].vy;
        l1z3[i] = dt*M3[i].vz;
        k1m3[i] = dt*(planet_accretion(M3[i],M1)+planet_evap(M3[i]));
    ...
    ay2 = acc2(ay1,M1.m,M2.m);
    az2 = acc2(az1,M1.m,M2.m);
    k2x2 = dt*(ax2+Ff(M2,M1,P,0)+Fgx(M2,M1));
    k2y2 = dt*(ay2+Ff(M2,M1,P,1)+Fgy(M2,M1));
    k2z2 = dt*(az2+Ff(M2,M1,P,2)+Fgz(M2,M1));
    l2x2 = dt*M2.vx;
    l2y2 = dt*M2.vy;
    l2z2 = dt*M2.vz;
    k2m2 = dt*planet_accretion(M2,M1)+dt*planet_evap(M2);

    for(int i=0;i<N_ast;i++){
        ax3 = acc3x(M1,M2,M3[i],SIM[i]);
        ay3 = acc3y(M1,M2,M3[i],SIM[i]);
        az3 = acc3z(M1,M2,M3[i],SIM[i]);
        k2x3[i] = dt*(ax3+Ff(M3[i],M1,P,0)+Fgx(M3[i],M1));
        k2y3[i] = dt*(ay3+Ff(M3[i],M1,P,1)+Fgy(M3[i],M1));
        k2z3[i] = dt*(az3+Ff(M3[i],M1,P,2)+Fgz(M3[i],M1));
        l2x3[i] = dt*M3[i].vx;
        l2y3[i] = dt*M3[i].vy;
        l2z3[i] = dt*M3[i].vz;
        k2m3[i] = dt*(planet_accretion(M3[i],M1)+planet_evap(M3[i]));
    ...
    ay2 = acc2(ay1,M1.m,M2.m);
    az2 = acc2(az1,M1.m,M2.m);
    k3x2 = dt*(ax2+Ff(M2,M1,P,0)+Fgx(M2,M1));
    k3y2 = dt*(ay2+Ff(M2,M1,P,1)+Fgy(M2,M1));
    k3z2 = dt*(az2+Ff(M2,M1,P,2)+Fgz(M2,M1));
    l3x2 = dt*M2.vx;
    l3y2 = dt*M2.vy;
    l3z2 = dt*M2.vz;
    k3m2 = dt*(planet_accretion(M2,M1)+planet_evap(M2));

    for(int i=0;i<N_ast;i++){
        ax3 = acc3x(M1,M2,M3[i],SIM[i]);
        ay3 = acc3y(M1,M2,M3[i],SIM[i]);
        az3 = acc3z(M1,M2,M3[i],SIM[i]);
        k3x3[i] = dt*(ax3+Ff(M3[i],M1,P,0)+Fgx(M3[i],M1));
        k3y3[i] = dt*(ay3+Ff(M3[i],M1,P,1)+Fgy(M3[i],M1));
        k3z3[i] = dt*(az3+Ff(M3[i],M1,P,2)+Fgz(M3[i],M1));
        l3x3[i] = dt*M3[i].vx;
        l3y3[i] = dt*M3[i].vy;
        l3z3[i] = dt*M3[i].vz;
        k3m3[i] = dt*(planet_accretion(M3[i],M1)+planet_evap(M3[i]));
        M3[i].x = M3x[i] + l3x3[i];
        M3[i].y = M3y[i] + l3y3[i];
        M3[i].z = M3z[i] + l3z3[i];
    ...
    ay2 = acc2(ay1,M1.m,M2.m);
    az2 = acc2(az1,M1.m,M2.m);
    k4x2 = dt*(ax2+Ff(M2,M1,P,0)+Fgx(M2,M1));
    k4y2 = dt*(ay2+Ff(M2,M1,P,1)+Fgy(M2,M1));
    k4z2 = dt*(az2+Ff(M2,M1,P,2)+Fgz(M2,M1));
    l4x2 = dt*M2.vx;
    l4y2 = dt*M2.vy;
    l4z2 = dt*M2.vz;
    k4m2 = dt*(planet_accretion(M2,M1)+planet_evap(M2));

    for(int i=0;i<N_ast;i++){
        ax3 = acc3x(M1,M2,M3[i],SIM[i]);
        ay3 = acc3y(M1,M2,M3[i],SIM[i]);
        az3 = acc3z(M1,M2,M3[i],SIM[i]);
        k4x3[i] = dt*(ax3+Ff(M3[i],M1,P,0)+Fgx(M3[i],M1));
        k4y3[i] = dt*(ay3+Ff(M3[i],M1,P,1)+Fgy(M3[i],M1));
        k4z3[i] = dt*(az3+Ff(M3[i],M1,P,2)+Fgz(M3[i],M1));
        l4x3[i] = dt*M3[i].vx;
        l4y3[i] = dt*M3[i].vy;
        l4z3[i] = dt*M3[i].vz;
        k4m3[i] = dt*(planet_accretion(M3[i],M1)+planet_evap(M3[i]));
    }
    ...
    m2.m += 1./6. * (k1m2+2.*k2m2+2.*k3m2+k4m2);
    m2.T = M2.T;
    //m2.R = pow(m2.m*3./(m2.density*4.*pi),1./3.);
    ...
    for(int i=0;i<N_ast;i++){
        ...
        ast[i].m += 1./6. * (k1m3[i]+2.*k2m3[i]+2*k3m3[i]+k4m3[i]);
        ast[i].R = pow(ast[i].m*3./(ast[i].density*4.*pi),1./3.);
        ast[i].T = M3[i].T;
    }
    /*m2.rho = M3[0].rho;
    double Fgv = sqrt(Fgx(m2,m1)*Fgx(m2,m1)+Fgy(m2,m1)*Fgy(m2,m1)+Fgz(m2,m1)*Fgz(m2,m1))*m2.m;
    double Ffv = sqrt(Ff(m2,m1,P,0)*Ff(m2,m1,P,0)+Ff(m2,m1,P,1)*Ff(m2,m1,P,1)+Ff(m2,m1,P,2)*Ff(m2,m1,P,2))*m2.m;
    double v = sqrt((m2.vx-m1.vx)*(m2.vx-m1.vx)+(m2.vy-m1.vy)*(m2.vy-m1.vy)+(m2.vz-m1.vz)*(m2.vz-m1.vz));
    double mls = P.mls/31557600.;
    double mpev = planet_evap(m2);
    double macc = planet_accretion(m2,m1);
    double taux = -(-mls+mpev+macc)/(m1.m+m2.m) - 2.*(Ffv+Fgv)/(m2.m*v);
